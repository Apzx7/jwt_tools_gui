...
# 前言
写给曾经只会“点点点”的自己，也写给还在纠结要不要学命令行的小伙伴——

一开始我连 `sqlmap -u` 都嫌长，满世界搜“sqlmapGUI”，看到按钮就兴奋：终于不用背参数了！  
后来真的用上 GUI，才发现：  
- 窗口弹半天，结果卡死；  
- 想批量跑 200 个目标，按钮点到手抽筋；  
- 作者没提供的功能，只能干瞪眼。  

有一天我用一个浏览器插件，顺手想“要是能把数据丢给管道、awk、jq 多爽”，居然冒出“干脆自己写个 CLI 吧”的念头——**当年最痛恨的命令行，如今成了自由感来源**。  

原因不过两条：  
1. CLI 像乐高，能随意拆插，一条管道就是一条工作流；  
2. 脚本里改两行，就能去陪女朋友吃饭，而不是对着窗口重复点击。  

所以如果你现在还沉迷图形化，没问题，先让工具带你跑起来；但请给自己留一点“不舒服”：  
- 把 GUI 背后那条可执行命令复制出来，跑通一次；  
- 把官方文档的例子粘到终端里，看一次回显；  
- 把 AI 生成的脚本读一行改一行，试一次结果。  

哪天你发现“学的东西好像用不上”，也别沮丧——**那是昨天的你送给今天自己的礼物**；  
把握当下，哪怕只学一个参数，也是在给未来的自己铺路。  

最后，命令行不是目的，省出时间陪自己和自己心爱的人才是。
## 功能特点
